...
export class ContactService {
  ...
  constructor(private storage: StorageServiceService) {
    let contacts = storage.loadFromStorage(CONTACT_KEY)
    if (!contacts) contacts = CONTACTS;
    storage.saveToStorage(CONTACT_KEY, contacts)
    this._contacts$.next(contacts)
  }

  public loadContacts(filterBy = null): void {
    let contacts = this.storage.loadFromStorage(CONTACT_KEY)
  ...
  public deleteContact(id: string) {
    ...
    this.storage.saveToStorage(CONTACT_KEY, contacts)
    this._contacts$.next(contacts)
  }
